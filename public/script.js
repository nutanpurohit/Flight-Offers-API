...
const search = async () => {
  try {
    ...
    const response = await fetch(`/offers?${params}`);
    const data = await response.json();

    let newData = data.filter((object) => {
      let lastItiIndex = object.itineraries.length - 1;
      let lastSegInt = object.itineraries[lastItiIndex].segments.length - 1;
      let arrivalDate = object.itineraries[lastItiIndex].segments[
        lastSegInt
      ].arrival.at.split("T")[0];

      return arrivalDate === returnDateInput.value;
    });
    return newData;
  } catch (error) {
    console.error(error);
            ...
            const travelPath = itinerary.segments
              .flatMap(({ arrival, departure }, index, segments) => {
                ...
                if (index === segments.length - 1) {
                  arrivalTime = new Date(`${arrival?.at}`).toLocaleDateString();
...
searchButton.addEventListener("click", async () => {
  ...
  if (document.getElementById("total-flights")) {
    document.getElementById("total-flights").remove();
  }
